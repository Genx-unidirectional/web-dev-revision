...
function checkNumPalindrome(n: number) {
  let copy = n;
  let reversedNum = 0;
  while (copy > 0) {
...
//using recursion
function fibOfNum(n: number): any {
  if (n == 1) {
    return 0;
  }
  if (n == 2) {
...
function identifyMissing(arr: number[]) {
  let missingNum;
...
function identifyMissing2(arr: number[]) {
  let sum = 0;
  for (let i = 0; i < arr.length; i++) {
    sum += arr[i];
  }
...
function findCountOfOdd(low: number, high: number) {
  let count = 0;
...
function itsPowerOfTwo(n: number) {
  if (n <= 0) return false;
